...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String sql2 = "insert into g_artists (name, a_type, a_year, debut, regdate)" + "values ('엑소', '남성', '2010년대', '2012년', datetime('now', 'localtime'));";
			int cnt = stat2.executeUpdate(sql2);
			if(cnt > 0)
				System.out.println("새로운 데이터가 추가되었습니다!");
			else
				System.out.println("[Error] 데이터 추가 오류 발생! ");
			stat2.close();
			
			//데이터 수정
			System.out.println("\n*** 데이터 수정 ***");
			Statement stat3 = con.createStatement();
			String sql3 = "update g_artists set a_year = '2000년, 2010년대, 2020년대'" + "where id=1 ;";
